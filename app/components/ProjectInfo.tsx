...
const ProjectInfo: React.FC<ProjectInfoProps> = (props) => {
  return (
    <motion.div
      ...
      style={{ opacity: props.opacityContent }}
    >
      {/* Project Title */}
      <div className="flex items-center gap-2">
        <h2 className="relative lg:text-3xl text-2xl bg-gradient-to-r bg-clip-text text-transparent from-blue-400 to-blue-200 font-semibold lg:before:content-[''] lg:before:h-1 lg:before:w-[25px] lg:before:rounded-full lg:before:bg-gradient-to-r  before:from-blue-600 before:to-blue-200 lg:before:absolute lg:before:top-1/2 lg:before:-left-[35px]">{props.name}</h2>
        <a href={props.github} target="_blank" className="hover:scale-110 transition-all duration-300 p-1">
      ...
      <ul className="flex flex-col gap-1.5">
        {props.features?.map((feat, idx) => (
          <li key={idx} className="flex items-center gap-1 text-sm">
            <MdNavigateNext className="text-xl text-blue-400" />
            {feat}
          </li>
        ))}
      </ul>
      <motion.div className="flex flex-wrap gap-3" variants={variants} initial="hidden" whileInView="visible">
