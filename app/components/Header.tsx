    ...
    navItems.map((item) => (
      <button
        ...
        className={`w-full relative cursor-pointer text-lg ${
          active === item
            ? 'bg-blue-400/50 after:content-[""] after:absolute sm:after:h-0.5 after:w-1/3 sm:after:bg-white after:-top-1.75 after:left-1/2 after:-translate-x-1/2 after:rounded-t-full sm:after:shadow-[0_0_10px_6px_rgba(255,255,255,0.6)]'
            : 'bg-transparent'
  ...
  return (
    <header className="h-[100px] mx-4 sticky z-[999] top-0 left-0 flex flex-col items-center">
      ...
      <motion.nav 
        ...
        animate={{ y: 0, opacity: 1 }}
        transition={{ duration: 0.5, ease: 'easeInOut' }}
        className="hidden sm:grid grid-cols-5 items-center p-1 border border-blue-700 bg-blue-900/10 backdrop-blur-md rounded-full mt-6">
          <motion.div
